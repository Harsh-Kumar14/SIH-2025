...
import type {DoctorDocument}  from './Doctor/doctorModel.js';
...
} from './consultation/consultationService.js';
import { ConsultationSchemaZod, ConsultationStatus } from './consultation/consultationModel.js';
import type { BookConsultationData, UpdateConsultationStatus } from './consultation/consultationService.js';
...
app.get('/get-users-by-doctor/:doctorId', async (req: Request, res: Response) => {
  try {
...
app.post('/prescribe-medicine/:userId', async (req: Request, res: Response) => {
  try {
...
app.post('/prescribe-multiple-medicines/:userId', async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
app.get('/get-prescribed-medicines/:userId', async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
...
app.get('/get-users-by-medicine/:medicineName', async(req: Request, res: Response) => {
  try {
    ...
    const users = await getUsersByMedicine(medicineName as string);
...
// Consultation routes
app.post('/book-consultation', async (req: Request, res: Response) => {
  try {
...
app.put('/update-consultation-status', async (req: Request, res: Response) => {
  try {
    ...
    const updateData: UpdateConsultationStatus = {
      doctorId,
      patientId,
      status,
      doctorNotes
...
app.get('/patient-consultation-history/:patientId', async (req: Request, res: Response) => {
  try {
...
app.get('/consultations-by-status/:doctorId/:status', async (req: Request, res: Response) => {
  try {
    ...
    const consultations = await getConsultationsByStatus(doctorId, status as ConsultationStatus);
    res.status(200).json(consultations);
  } catch (error) {
...
app.delete('/cancel-consultation/:doctorId/:patientId', async (req: Request, res: Response) => {
  try {
...
app.get('/next-patient/:doctorId', async (req: Request, res: Response) => {
  try {
    const doctorId = req.params.doctorId;
    if (!doctorId) {
      return res.status(400).json({ error: 'Doctor ID is required' });
...
app.get('/consultation-stats', async (req: Request, res: Response) => {
  try {
...
app.get('/consultation-stats/:doctorId', async (req: Request, res: Response) => {
  try {
    const doctorId = req.params.doctorId;
...
app.get('/pharmacy/medicines/low-stock', async (req: Request, res: Response) => {
  try {
...
app.get('/pharmacy/medicines/expiring', async (req: Request, res: Response) => {
  try {
...
// Notification routes
app.get('/notifications/low-stock-users', async (req: Request, res: Response) => {
  try {
    const threshold = req.query.threshold ? parseInt(req.query.threshold as string) : 10;
...
app.get('/notifications/expiring-users', async (req: Request, res: Response) => {
  try {
    const days = req.query.days ? parseInt(req.query.days as string) : 30;
...
app.get('/notifications/summary', async (req: Request, res: Response) => {
  try {
    const lowStockThreshold = req.query.threshold ? parseInt(req.query.threshold as string) : 10;
    const expiringDays = req.query.days ? parseInt(req.query.days as string) : 30;
